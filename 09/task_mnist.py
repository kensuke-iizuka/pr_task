...
def Read_data( flag ):
  ...
  for i in range(class_num):
    for j in range(1,train_num+1):
      ...
      data_vec[i][j-1][0] = work_array[:,:,0].flatten() # Red image
      data_vec[i][j-1][1] = work_array[:,:,1].flatten() # Green image
      data_vec[i][j-1][2] = work_array[:,:,2].flatten() # Blue image
      # 入力値の合計を1とする
      data_vec[i][j-1][0] = data_vec[i][j-1][0] / 255 
      data_vec[i][j-1][1] = data_vec[i][j-1][1] / 255 
      data_vec[i][j-1][2] = data_vec[i][j-1][2] / 255 
...
def Train():
  ...
  # Save weights
  r_outunit.Save( "dat/mnist-r-out.npz" )
  r_hunit.Save( "dat/mnist-r-hunit.npz" )
  g_outunit.Save( "dat/mnist-g-out.npz" )
  g_hunit.Save( "dat/mnist-g-hunit.npz" )
  b_outunit.Save( "dat/mnist-b-out.npz" )
  b_hunit.Save( "dat/mnist-b-hunit.npz" )


def Predict():
  # Load weights
  r_outunit.Load( "dat/mnist-r-out.npz" )
  r_hunit.Load( "dat/mnist-r-hunit.npz" )
  g_outunit.Load( "dat/mnist-g-out.npz" )
  g_hunit.Load( "dat/mnist-g-hunit.npz" )
  b_outunit.Load( "dat/mnist-b-out.npz" )
  b_hunit.Load( "dat/mnist-b-hunit.npz" )
  # 混合行列
  ...
  Visualize_weights(r_hunit, "mnist-r_hunit")
  Visualize_weights(g_hunit, "mnist-g_hunit")
  Visualize_weights(b_hunit, "mnist-b_hunit")
